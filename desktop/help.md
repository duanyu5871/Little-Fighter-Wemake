...
- 托盘菜单（任务栏右下角图标）：
  ...
  - 允许局域网连接：勾选后服务器改听 `0.0.0.0`，同一网络下的其他人可用「复制联机地址」得到的地址连入（切换会自动重启服务器）
  - 复制联机地址 / 打开数据工具（命令行）/ 复制数据工具命令 / 打开数据目录 / 显示游戏窗口 / 退出
...
| `--debug` | 打印收到的弹幕事件（调试用） |
| `--devtools` | 打开开发者工具 |
| `--screenshot <path>` | 启动后把游戏画面截图保存为 PNG |
| `--host <host>` | 监听地址，默认 `127.0.0.1`；改成 `0.0.0.0` 可让局域网直接访问游戏画面与弹幕桥 |
| `--port <port>` | 弹幕桥端口，默认 8066（被占用时会提示并退出） |
| `--game-port <port>` | 游戏画面端口，默认 8067（被占用时自动向后找空位） |
...
- 端口被占用时可以 `--server-port <port>` 换一个；切换「允许局域网连接」时服务器会自动重启
...
- 游戏画面默认也只监听本机；`start.exe --host 0.0.0.0` 后，同一网络下可以直接用浏览器打开 `http://<你的IP>:8067/` 玩你这份游戏
...
| 端口被占用 | `--port` / `--game-port` / `--server-port` 可以改；弹幕桥端口被占用会直接退出并提示 |
