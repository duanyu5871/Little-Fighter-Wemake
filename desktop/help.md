...
- 托盘菜单（任务栏右下角图标）：
  ...
  - 其他：打开数据工具（命令行）/ 复制数据工具命令 / 打开数据目录 / 显示游戏窗口 / 退出
  - 菜单语言跟随游戏内语言（游戏为英文时托盘也是英文）；想固定用启动参数 `--lang`；自定义文案放在本目录 `langs\` 文件夹（每语言一个文件，写法见 `langs\README.txt`）
...
| `--lang <code>` | 界面（托盘）语言：`auto` 跟随游戏内语言（默认）/ `zh-hans` / `zh-hant` / `en`；其他语言码需在 `langs\` 里提供文案，否则用英文；固定后不再跟随游戏切换 |
